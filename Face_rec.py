...
def logger(identifier, distance, duration):
  if (identifier == '-'):
    print('Unknown person, operation took', duration, 'seconds.')
  elif (identifier == None):
    print('No face recognised, operation took',duration, 'seconds.')
  else:
    print('The person in question is', identifier, 'with calculated distance of', distance, ',operation took', duration, 'seconds.')
...
def loadTestImages():
  print('Enrolling all test images')
  for filename in glob.glob('test_faces/*.jpg'):
    ...
    enroll_face(image, name)
    print('Enrolled', name)
  np.save('test_faces.npy', enrolled_faces)
  print('Saved test_faces.npy')

#The main script that runs the image enrolling and testing
#To reset the faces, delete faces.npy from the script's folder
def main():
